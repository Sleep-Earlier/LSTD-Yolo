...
from ultralytics.nn.extra_modules.MyPruner import DiverseBranchBlockPruner, RepConvPruner, DyHeadBlockPruner
from timm.models.layers import SqueezeExcite
from ultralytics.nn.extra_modules.block import GLU_SE, GLU
...
def get_pruner(opt, model, example_inputs):
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, Detect):
            ignored_layers.append(module.cv2[0][2])
            ignored_layers.append(module.cv2[1][2])
            ignored_layers.append(module.cv2[2][2])
            ignored_layers.append(module.cv2[3][2])
            ignored_layers.append(module.cv3[0][2])
            ignored_layers.append(module.cv3[1][2])
            ignored_layers.append(module.cv3[2][2])
            ignored_layers.append(module.cv3[3][2])
            ignored_layers.append(module.dfl)

        elif isinstance(module, PSABlock):
            ignored_layers.append(module.attn)

        elif isinstance(module, AAttn):
            ignored_layers.append(module)

        # 精准忽略 GLU_SE 中 dwconv 的 Conv2d 子层
        elif isinstance(module, GLU):
            if hasattr(module, 'dwconv'):
                for sub_name, sub_module in module.dwconv.named_modules():
